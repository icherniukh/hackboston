...
public final class MessageControllerImpl: MessageController {
    ...
    public init(message: String, genre: String, source: MessageSource, songService: MessageToSongService, downloadService: SongDownloadService) {
    ...
    public func send() async throws -> LoadedSong {
        let remoteMessage = try await songService.generateSong(message: message, genre: genre)
